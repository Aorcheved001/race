...
```
本项目允许使用 Git 命令和 GitMCP 做版本管理，但必须遵守以下边界：
...
- git commit
- git push
- git switch / git checkout（仅切换到已知安全分支）
- git config（仅限用户名、邮箱、代理、alias 等本地配置）
...
- git rebase 涉及改写已发布历史
- git cherry-pick / revert 大范围批量操作
- 删除分支、覆盖远程分支、回滚共享历史

其他要求：
- 不得因为方便而擅自改写用户历史提交。
- 不得在未确认的情况下 amend 用户刚创建的提交。
- 如果仓库工作区是脏的，先检查是否存在用户未提交的重要改动，再继续 Git 操作。
- 在 PowerShell 中执行 Git 命令时，严禁把示例占位符尖括号 < > 原样输入。
```
...
- 禁止把 Git 教程中的 <branch>、<file>、<message> 等占位符原样复制到 PowerShell 中执行
```

### 4.3 GitHub 连通性约束

```
当前环境下 GitHub 连通性可能不稳定，处理顺序必须如下：
1. 先检查 remote.origin.url 是否正确。
2. 再测试 github.com:443 是否可直连。
3. 若直连失败且 Clash 已启动，优先给 Git 配置代理：
   git config --global http.proxy http://127.0.0.1:7890
   git config --global https.proxy http://127.0.0.1:7890
4. 使用 git ls-remote origin 验证远程是否可达后，再执行 git push。
5. 若确认直连已稳定可用，再移除 Git 代理。
```
