...
```
new_ins/
©À©¤©¤ code/                    Application source code (3 modules)
©¦   ©À©¤©¤ system/              System init, interrupt scheduler, menu
©¦   ©À©¤©¤ ins/                 INS (Inertial Navigation System) module
©¦   ©¸©¤©¤ control/             Motor, PID, encoder, servo, track, remote
©À©¤©¤ libraries/               Third-party libraries (ZF driver + Infineon iLLD)
©¦   ©À©¤©¤ zf_common/           ZF common utilities (FIFO, debug, font, etc.)
©¦   ©À©¤©¤ zf_driver/           ZF peripheral drivers (GPIO, UART, PWM, etc.)
©¦   ©À©¤©¤ zf_device/           ZF device drivers (sensors, displays, wireless)
©¦   ©À©¤©¤ zf_components/       ZF components (printf redirect, assistant)
©¦   ©¸©¤©¤ infineon_libraries/  Infineon iLLD low-level drivers for TC37A
©À©¤©¤ Debug/                   Build output (.elf, .hex, .map, .o files)
©À©¤©¤ tools/                   Build/flash/sync scripts
©À©¤©¤ scripts/                 Python tools (serial reader, analyzers)
©À©¤©¤ data/                    Serial port collected data
©À©¤©¤ docs/                    Documentation
©À©¤©¤ .cproject / .project     ADS/Eclipse project configuration
...
```
core0_main()
  ¡ú clock_init()              // Get clock frequency (MUST keep)
  ¡ú debug_init()              // Init debug UART
  ¡ú system_init_all()         // [init_all.c] All peripherals init
  ¡ú cpu_wait_event_ready()    // Wait all cores ready
  ¡ú while(1) {
...
```
system_init_all()  ¡ú  code/system/init_all.c:8
  ¡ú imu963ra_init()          // IMU sensor init
  ¡ú imu_bias_init()           // IMU bias calibration
  ¡ú imu_gyro_z_autocalib()    // Gyro Z-axis auto-calibration
  ¡ú imu_mag_bias_load()       // Magnetometer calibration params
  ¡ú ips200_init()             // LCD display init (SPI mode)
  ¡ú gnss_init(TAU1201)        // GNSS/GPS receiver init
  ¡ú key_init(8)               // Button/key scanner init (8 keys)
  ¡ú encoder_init()            // Wheel encoder init
  ¡ú encoder_layer_set_model() // Encoder tick-to-meter model
  ¡ú wireless_uart_init()      // COM24 wireless UART (115200bps)
  ¡ú motor_init()              // Motor PWM/direction pins
  ¡ú servo_init()              // Servo control init
  ¡ú yaokong_init(0.8f)        // Remote control receiver init
  ¡ú Ins_init()                // INS navigation system init
  ¡ú INS_init()                // INS state machine init
  ¡ú pit_ms_init(CCU60_CH0, 1) // 1ms periodic timer (CCU6)
  ¡ú wheel_pid_init()          // Speed PID controller init
  ¡ú track_init()              // Trajectory tracking init
```
...
©À©¤©¤ Cpu/Std/           IfxCpu (CPU core control)
©À©¤©¤ Gtm/Std/           IfxGtm (Generic Timer Module)
©À©¤©¤ Gtm/Atom/Pwm/      IfxGtm_Atom_Pwm (PWM generation)
©À©¤©¤ Gpt12/Std/         IfxGpt12 (General Purpose Timer 12)
©À©¤©¤ Port/Std/          IfxPort (Port I/O pins)
©À©¤©¤ Scu/Std/           IfxScu (System Control Unit)
©À©¤©¤ Src/Std/           IfxSrc (Service Request Generator)
©À©¤©¤ Stm/Std/           IfxStm (System Timer Module)
©À©¤©¤ Asclin/Std/        IfxAsclin (ASC LIN Serial Interface = UART)
©À©¤©¤ Asclin/Asc/        IfxAsclin_Asc (ASC sub-module)
©À©¤©¤ Qspi/Std/          IfxQspi (Quad SPI)
©À©¤©¤ Evadc/Std/         IfxEvadc (Enhanced ADC)
©À©¤©¤ Dma/Std/           IfxDma (DMA)
©À©¤©¤ Flash/Std/         IfxFlash (Flash memory)
©À©¤©¤ _Impl/             Configuration implementations (_cfg.c files)
©À©¤©¤ _PinMap/           Pin mapping definitions
©À©¤©¤ Configurations/    Ifx_Cfg_Ssw (Software Switch)
...
- **Source**: `code/system/interrupt.c` ¡ú `send_pos_to_host()` ¡ú `wireless_uart_send_string()`
