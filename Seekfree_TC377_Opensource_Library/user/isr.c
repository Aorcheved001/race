...
unsigned char flag_4ms = 0;
unsigned char flag_8ms = 0;
unsigned char flag_10ms = 0;
unsigned char flag_16ms = 0;
unsigned char flag_20ms = 0;
unsigned char flag_40ms = 0;
...
//    }
//    task_4ms();
    flag_1ms++;  flag_2ms++;  flag_4ms++;  flag_8ms++;
    flag_10ms++; flag_16ms++; flag_20ms++; flag_40ms++;
//    printf("%d\r\n", flag_4ms);
    if (flag_1ms == 1)  { task_1ms();  flag_1ms = 0;  }
    if (flag_2ms == 2)  { task_2ms();  flag_2ms = 0;  }
    if (flag_4ms == 4)  { task_4ms();  flag_4ms = 0;  }
    if (flag_8ms == 8)  { task_8ms();  flag_8ms = 0;  }
    if (flag_10ms == 10){ task_10ms(); flag_10ms = 0; }
    if (flag_16ms == 16){ task_16ms(); flag_16ms = 0; }
    if (flag_20ms == 20){ task_20ms(); flag_20ms = 0; }
    if (flag_40ms == 40){ task_40ms(); flag_40ms = 0; }
...
// 串口5连接到LORA3A22遥控器模块
IFX_INTERRUPT(uart5_tx_isr, 0, UART5_TX_INT_PRIO)
{
    interrupt_global_enable(0);                     // 开启中断嵌套
}

IFX_INTERRUPT(uart5_rx_isr, 0, UART5_RX_INT_PRIO)
{
    interrupt_global_enable(0);                     // 开启中断嵌套
    lora3a22_uart_callback();                       // LORA3A22遥控器串口回调
}

IFX_INTERRUPT(uart5_er_isr, 0, UART5_ER_INT_PRIO)
{
    interrupt_global_enable(0);                     // 开启中断嵌套
