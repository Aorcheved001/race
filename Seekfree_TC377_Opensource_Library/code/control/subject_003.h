...
typedef enum {
    ...
    STATE_CHANGE,             // 遥控调头：遥控控制调转车头方向，INS持续运行，坐标实时发送
    STATE_YAW_CORRECT,        // 航向矫正：车辆静止，使用LUT后磁航向/RTK监督纠正yaw漂移
...
/**
 * @brief 科目三显示刷新（40ms调用）
 * @note  在IPS200屏幕上显示状态机状态、INS航向、坐标、速度等信息
 */
