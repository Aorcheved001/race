...
typedef struct
{
    ...
    int16 tick_left_4ms;                    // 【新增】左轮每 4ms 脉冲值（供新 PID 使用）
    int16 tick_right_4ms;                   // 【新增】右轮每 4ms 脉冲值（供新 PID 使用）
    ...
    int32 total_tick_left;                  // 自系统启动以来的左轮累计脉冲（正=前进方向）
    int32 total_tick_right;                 // 自系统启动以来的右轮累计脉冲（正=前进方向）
    float speed_left_mps;                   // 左轮当前周期瞬时速度（m/s）
    float speed_right_mps;                  // 右轮当前周期瞬时速度（m/s）
    float speed_average_mps;                // 左右轮速度算术平均（m/s）
} EncoderLayerState;

extern EncoderLayerState g_state;


// === 双核安全环形缓冲区（SPSC, 无锁） ===
#define ENC_RING_BUF_SIZE  8  // 必须是2的幂，位与替代取模

typedef struct
{
    float delta_left_m;          // 左轮位移增量（m）
    float delta_right_m;         // 右轮位移增量（m）
    int16 tick_left;             // 原始左轮脉冲（供 PID 用）
    int16 tick_right;            // 原始右轮脉冲（供 PID 用）
    float speed_avg_mps;         // 平均速度（m/s）
} EncoderRingEntry;

// 消费者接口：Core0 主循环调用，从环形缓冲区取出一个采样
// 返回 1=有数据, 0=缓冲区空
uint8 encoder_ring_pop(EncoderRingEntry* out);

// 获取环形缓冲区当前积压数量（未消费的采样条数）
// 用于调试：观察 Core0 是否追得上 Core1 的节拍
