...
////-------------------------------------------------------------------------------------------------------------------
////  @brief      静止状态下自动校准三轴陀螺仪零偏 + 加速度计零偏
////  @param      data_src       IMU 参数结构体指针
////  @param      samples        采样次数
////  @return     void
////  @note       静止状态下取平均值作为零偏；加速度计 Z 轴减去 1g 重力
////-------------------------------------------------------------------------------------------------------------------
void imu_gyro_z_autocalib(imu_param *data_src, unsigned int samples)
{
    ...
    for (unsigned int i = 0; i < samples; i++) {
        imu963ra_get_gyro();
        imu963ra_get_acc();
        sum_gx += (float)imu963ra_gyro_x;
        sum_gy += (float)imu963ra_gyro_y;
        sum_gz += (float)imu963ra_gyro_z;
        sum_ax += (float)imu963ra_acc_x;
        sum_ay += (float)imu963ra_acc_y;
        sum_az += (float)imu963ra_acc_z;
        system_delay_ms(2);
    }

    // 三轴陀螺仪零偏：静止时角速度应为0，均值即为零偏
    data_src->gyro_x_bias = sum_gx / (float)samples;
    data_src->gyro_y_bias = sum_gy / (float)samples;
    data_src->gyro_z_bias = sum_gz / (float)samples;

    // 加速度计零偏：静止水平时 acc_x≈0, acc_y≈0, acc_z≈1g(原始ADC值)
    // X/Y 轴零偏 = 均值（去偏公式用减号，减去均值后≈0）
    data_src->acc_x_bias = sum_ax / (float)samples;
    data_src->acc_y_bias = sum_ay / (float)samples;
    // Z 轴去偏公式是 acc_z + acc_z_bias，需要结果≈1g_raw
    // acc_z_bias = 0 意味着不修正Z轴（卡尔曼滤波器对Z轴零偏不敏感）
...
////-------------------------------------------------------------------------------------------------------------------
////  @brief      将校准后的磁力计数据发送到 PC 端（用于验证离线校准效果）
////  @param      void
////  @return     void
////-------------------------------------------------------------------------------------------------------------------
void imu_mag_send_calibrated_data_to_pc(void)
{
    char send_buffer[64];
    sprintf(send_buffer, "%.2f,%.2f,%.2f\r\n", imu660.data_Ripen.mag_x, imu660.data_Ripen.mag_y, imu660.data_Ripen.mag_z);
    wireless_uart_send_string(send_buffer);
}
////-------------------------------------------------------------------------------------------------------------------
////  @brief      将 yaw 分层与校准后磁力计数据发送到 PC 端（用于航向融合分析）
////  @param      void
////  @return     void
////  @note       旧格式: yawdbg:yaw_gyro_deg,yaw_mag_raw_deg,yaw_mag_rel_deg,yaw_fused_deg,mag_x,mag_y,mag_z
////              新格式: yawrtk:seq,tick_ms,phase,yaw_gyro,mag_yaw_raw,mag_yaw_rel,mag_yaw_corr_rel,yaw_fused,rtk_yaw_ins,rtk_yaw_ref,rtk_valid,mag_x,mag_y,mag_z
////-------------------------------------------------------------------------------------------------------------------
void imu_mag_send_yaw_debug_to_pc(void)
{
    char send_buffer[256];
    float yaw_gyro = 0.0f;
    float mag_yaw_raw = 0.0f;
    float mag_yaw_rel = 0.0f;
    float mag_yaw_corr_rel = 0.0f;
    float yaw_fused = 0.0f;
    float mag_x = 0.0f, mag_y = 0.0f, mag_z = 0.0f;

    // 获取含 LUT 修正的调试 yaw 数据
    Ins_get_yaw_debug_layers(&yaw_gyro, &mag_yaw_raw, &mag_yaw_rel,
                              &mag_yaw_corr_rel, &yaw_fused);
    Ins_get_mag_vector(&mag_x, &mag_y, &mag_z);

    // 获取 RTK 数据
    const rtk_state_t *rtk = rtk_get_state();
    float rtk_yaw_ins = 0.0f;
    float rtk_yaw_ref = 0.0f;
    uint8_t rtk_valid = 0u;
    if(rtk != NULL)
    {
        rtk_yaw_ins = rtk->yaw_ins * RTK_RAD_TO_DEG;
        rtk_yaw_ref = rtk_get_yaw_ref_deg();
        if(rtk->pos_valid != 0u) rtk_valid |= 0x01u;
        if(rtk->yaw_valid != 0u) rtk_valid |= 0x02u;
    }

    // 获取科目三阶段
    Subject3_State_t phase = subject3_get_state();
    const char *phase_str = "IDLE ";
    switch(phase)
    {
        case STATE_IDLE:        phase_str = "IDLE "; break;
        case STATE_DRIVE:       phase_str = "DRIVE"; break;
        case STATE_CHANGE:      phase_str = "CHANG"; break;
        case STATE_YAW_CORRECT: phase_str = "YAWCR"; break;
        case STATE_FOLLOW:      phase_str = "FOLLO"; break;
        case STATE_ARRIVE:      phase_str = "ARRIV"; break;
        case STATE_ERROR:       phase_str = "ERROR"; break;
        default:                phase_str = "UNKN "; break;
    }

    // 静态序号（tick从1ms中断获取，不再本地累加）
    static uint16_t s_seq = 0u;
    s_seq++;
    uint32_t tick_ms = debug_layer_get_tick_ms();  // 使用中断维护的时间戳

    sprintf(send_buffer,
            "yawrtk:%u,%lu,%s,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%u,%.2f,%.2f,%.2f\r\n",
            s_seq,
            (unsigned long)tick_ms,  // 使用中断时间戳
            phase_str,
            yaw_gyro * INS_RAD2DEG,
            mag_yaw_raw * INS_RAD2DEG,
            mag_yaw_rel * INS_RAD2DEG,
            mag_yaw_corr_rel * INS_RAD2DEG,
            yaw_fused * INS_RAD2DEG,
            rtk_yaw_ins,
            rtk_yaw_ref,
            rtk_valid,
            mag_x,
...
////-------------------------------------------------------------------------------------------------------------------
////  @brief      发送两路纯陀螺 yaw 对比数据到 PC 端
////  @param      void
////  @return     void
////  @note       gyroyaw:seq,z_bias_deg,rp_bias_deg,model_yaw_deg,roll_deg,pitch_deg,steer_deg,v_mps,raw,diff
////              z_bias_deg  = 当前主链路：Z轴陀螺零偏修正后积分yaw
////              rp_bias_deg = 调试链路：roll/pitch补偿角速度再零偏修正后积分yaw
////              raw = 编码器原始值, diff = 相对原位差值(跨零点已处理)
////-------------------------------------------------------------------------------------------------------------------
void imu_send_gyro_yaw_compare_to_pc(void)
{
    char send_buffer[160];
    float yaw_z_bias = 0.0f;
    float yaw_rp_bias = 0.0f;
    float yaw_model = 0.0f;
    float roll = 0.0f;
    float pitch = 0.0f;
    ...
    Ins_get_gyro_yaw_compare(&yaw_z_bias, &yaw_rp_bias, &yaw_model);
    Ins_get_attitude(&roll, &pitch, NULL);
    steer_deg = steering_get_current_angle();
    steering_get_raw_and_diff(&steer_raw, &steer_diff);
    v_mps = g_state.speed_average_mps;
    s_seq++;

    sprintf(send_buffer,
            "gyroyaw:%u,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.3f,%d,%ld\r\n",
            s_seq,
            yaw_z_bias * INS_RAD2DEG,
            yaw_rp_bias * INS_RAD2DEG,
            yaw_model * INS_RAD2DEG,
            roll * INS_RAD2DEG,
            pitch * INS_RAD2DEG,
            steer_deg,
            v_mps,
