...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     NMEA ddmm.mmmmmmmm 经纬度转为 deg * 1e9 整数
// 参数说明     *s              经纬度字段字符串
// 参数说明     degree_digits   纬度为2，经度为3
// 返回参数     int64           度 * 1e9
// 备注信息     避免TC377上绝对经纬度float/double精度不足导致RTK轨迹量化
//-------------------------------------------------------------------------------------------------------------------
static int64 get_nmea_deg_e9 (char *s, uint8 degree_digits)
{
    int64 degree = 0;
    int64 minute_int = 0;
    int64 minute_frac = 0;
    ...
    {
        s++;
        while(('0' <= *s) && ('9' >= *s) && (frac_digits < 9))
        {
            minute_frac = minute_frac * 10 + (*s - '0');
            scale *= 10;
            frac_digits++;
            s++;
        }
    ...
    while(scale < 1000000000LL)
    {
        minute_frac *= 10;
        scale *= 10;
    }

    // minute_e9单位为 minute * 1e9，除以60后得到 degree * 1e9 的小数度部分。
    int64 minute_e9 = minute_int * 1000000000LL + minute_frac;
    return degree * 1000000000LL + (minute_e9 + 30LL) / 60LL;
}
...
static uint8 gps_gnrmc_parse (char *line, gnss_info_struct *gnss)
{
    ...
    if('A' == state || 'D' == state)                                            // 如果数据有效 则解析数据
    {
        ...
        gnss->latitude_deg_e9   = get_nmea_deg_e9(&buf[get_parameter_index(3, buf)], 2);
        gnss->longitude_deg_e9  = get_nmea_deg_e9(&buf[get_parameter_index(5, buf)], 3);
        if('S' == gnss->ns) gnss->latitude_deg_e9 = -gnss->latitude_deg_e9;
        if('W' == gnss->ew) gnss->longitude_deg_e9 = -gnss->longitude_deg_e9;
        gnss->latlon_high_precision_valid = 1;
    ...
    {
        gnss->state = 0;
        gnss->latlon_high_precision_valid = 0;
    }
...
static uint8 gps_gngga_parse (char *line, gnss_info_struct *gnss)
{
    ...
    double latitude = 0;
    double longitude = 0;
    double lati_cent_tmp = 0;
    double long_cent_tmp = 0;

    state = buf[get_parameter_index(2, buf)];

    if(',' != state)
    {
        gnss->fix_quality    = (uint8)get_int_number(&buf[get_parameter_index(6, buf)]);  // GGA定位质量: 0=无效 1=单点 2=差分 4=RTK固定 5=RTK浮点
        ...
        if(0u != gnss->fix_quality)
        {
            gnss->ns = buf[get_parameter_index(3, buf)];
            gnss->ew = buf[get_parameter_index(5, buf)];

            latitude  = get_double_number(&buf[get_parameter_index(2, buf)]);
            longitude = get_double_number(&buf[get_parameter_index(4, buf)]);

            gnss->latitude_deg_e9  = get_nmea_deg_e9(&buf[get_parameter_index(2, buf)], 2);
            gnss->longitude_deg_e9 = get_nmea_deg_e9(&buf[get_parameter_index(4, buf)], 3);
            if('S' == gnss->ns) gnss->latitude_deg_e9 = -gnss->latitude_deg_e9;
            if('W' == gnss->ew) gnss->longitude_deg_e9 = -gnss->longitude_deg_e9;
            gnss->latlon_high_precision_valid = 1;
            ...
            lati_cent_tmp          = (latitude - gnss->latitude_degree * 100);
            gnss->latitude_cent    = (int)lati_cent_tmp;
            gnss->latitude_second  = (int)((lati_cent_tmp - gnss->latitude_cent) * 6000);

            gnss->longitude_degree = (int)longitude / 100;
            long_cent_tmp          = (longitude - gnss->longitude_degree * 100);
            gnss->longitude_cent   = (int)long_cent_tmp;
            gnss->longitude_second = (int)((long_cent_tmp - gnss->longitude_cent) * 6000);
