...
typedef struct
{
    ...
    int64       latitude_deg_e9;                                                // 高精度纬度，单位1e-9度，避免RTK小位移被float/double量化
    int64       longitude_deg_e9;                                               // 高精度经度，单位1e-9度，避免RTK小位移被float/double量化
    uint8       latlon_high_precision_valid;                                    // 高精度整数经纬度是否有效
    ...
    uint8       fix_quality;                                                    // GGA定位质量: 0=无效 1=单点 2=差分 4=RTK固定 5=RTK浮点
    float       height;                                                         // 高度
}gnss_info_struct;
