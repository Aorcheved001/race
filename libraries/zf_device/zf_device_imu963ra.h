...
=======
#define IMU963RA_MAG_SAMPLE                         (0x0D)                      // QMC5883L CONTROL1: OSR=64, ODR=200Hz, Continuous
// 0x0D = 0b00001101: OSR[1:0]=00(64x), ODR[1:0]=11(200Hz), MODE[1:0]=01(Continuous)
// 64x OSR @ 400Hz internal = 160ms per sample, compatible with 200Hz ODR
// 设置为:0x0D 磁力计量程为:8G     获取到的磁力计数据 除以3000， 可以转化为带物理单位的数据，单位：G(高斯)
// 设置为:0x09 磁力计量程为:2G     获取到的磁力计数据 除以12000，可以转化为带物理单位的数据，单位：G(高斯)
