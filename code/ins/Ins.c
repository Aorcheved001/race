...
void Ins_update(const INS_Input *input, float dt_s)
{
    ...
    if(input->mag_valid)
    {
        ...
        // 对相对磁航向做一次低通滤波，兼顾响应和稳定性
        // Alpha 从 0.1 提到 0.3 后，转向响应更快，抖动仍可接受
        float diff = normalize_angle_rad(s_mag_yaw_rel - s_mag_yaw_rel_filtered);
        s_mag_yaw_rel_filtered = normalize_angle_rad(s_mag_yaw_rel_filtered + s_config.mag_alpha * diff); // [P7-2-2] 使用配置参数
    ...
    // 参数使用说明：
    // - omega_rad_s: 当前版本未使用车辆模型角速度（待磁力计调试完成后启用）
