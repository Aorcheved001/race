...
#define IMU_MAG_BIAS_FLASH_PAGE      63u                                   // ????????????????Flash?
#define IMU_MAG_BIAS_MAGIC           0x4D414742u                           // ??????????"MAGB"
#define IMU_MAG_BIAS_VERSION         3u                                    // ??????????3(???3x3????????)

//-------------------------------------------???????-???????------------------------------------------------------------
static uint8 s_mag_calib_active = 0u;                                      // ?????????????????
static uint32 s_mag_calib_samples = 0u;                                    // ??????????????????
...
//-------------------------------------------???????-??????????------------------------------------------------------------
static uint8 s_mag_ellipsoid_active = 0u;                                 // ???????????????????
static float s_mag_samples[MAG_CALIB_MAX_SAMPLES][3];                     // ????????????????????
static uint32 s_mag_sample_count = 0u;                                     // ?????????????????
...
void imu_mag_bias_save(const imu_param *data_src)
{
    ...
    flash_buf[0] = IMU_MAG_BIAS_MAGIC;                                     // ??????????
    flash_buf[1] = IMU_MAG_BIAS_VERSION;                                   // ??????????
...
uint8 imu_mag_bias_load(imu_param *data_src)
{
    ...
    if(flash_buf[0] != IMU_MAG_BIAS_MAGIC) return 0u;                      // ??????????????
