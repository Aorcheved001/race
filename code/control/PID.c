...
/* PID输出限幅 - 与Motor.c接口匹配，percent参数范围0~100 */
#define WHEEL_PID_PWM_PERCENT_MAX    60.0f
#define WHEEL_PID_DEADZONE  4.0f     // 速度环死区，低于此值输出为0
#define WHEEL_PID_MIN_START_PWM   10.0f     // 最小启动PWM，克服电机静摩擦
...
// 返回参数     void
// 使用示例     wheel_pid_enable(1, 1, 0, 1);  // 输出? 速度环? 位置环? 角速度环?
// 备注信息     动态使能状态，支持多种模式
//-------------------------------------------------------------------------------------------------------------------
...
// 函数说明     target_speed_mps - 目标速度(m/s)，左右轮使用相同速度
// 返回参数     void
// 使用示例     wheel_pid_set_target_speed(1.0f);
// 备注信息     内部映射：wheel_pid_set_speed_target(speed, speed) — 左右轮使用相同速度
//-------------------------------------------------------------------------------------------------------------------
...
static void wheel_pid_update_cascade(const EncoderLayerState *enc, float yaw_rps, float dt_s)
{
    ...
    else if (!pos_on && spd_on && !yaw_on)
    {
        ...
        origin_error_right = (int16)err_spd_r;
        origin_error_left  = (int16)err_spd_l;
...
//        out_right = -WHEEL_PID_MIN_START_PWM;
    ...
    /* 电机输出 - motor_control参数为百分比(0~100) */
...
// 内部根据左右轮实际速度差自动估算yaw_rate，然后调用级联PID
// 当左右轮速度相同时（直行），估算yaw_rate = 0，与原单环PID行为一致
// 说明：使用编码器实际速度估算角速度，用于差速辅助转向
//-------------------------------------------------------------------------------------------------------------------
