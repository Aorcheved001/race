...
// PID 命令系统初始化（在 wheel_pid_init() 中被调用）
...
// 命令轮询处理（建议在 4ms 定时任务中调用）
void wheel_pid_cmd_poll(void)
{
    g_pid_cmd.timestamp_ms += 4u;   // 每调用一次累加4ms
...
// ...（后续的 cmd_set_xxx 函数与原来逻辑完全一致，仅注释改为中文）
...
void wheel_pid_cmd_start_step_test(float speed, uint32 duration_ms)
{
    ...
    wheel_pid_enable(1, 1, 0, 0);        // 仅开启速度环
