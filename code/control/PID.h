...
{
    float iError;                          // ��ǰ��� e(k)
    float LastError;                       // ��һ����� e(k-1)
    float SumError;                        // ������ ��e
} PID_INFO;

typedef struct
{
    PID_INFO pid_spd;                      // �ٶȻ� PID ״̬

    float speed_param[4];                  // �ٶȻ�������[0]=Kp [1]=Ki [2]=Kd [3]=�����޷�

    float target_speed_mps;                // Ŀ���ٶ�

    float speed_limit;                     // �ٶ��޷�

    float out_pwm;                         // ������� PWM�����Һ���ֵͬ��

    uint8 enable_output;                   // ���ʹ�ܣ�1=�·����PWM��0=���·�
    uint8 initialized;                     // ģ���ʼ����־��1=�ѳ�ʼ��
} WHEEL_PID_LAYER;
...
//-------------------------------------------����������------------------------------------------------------------
void pid_para_init(PID_INFO *pid_info);                     // ���� PID �ڲ�״̬

void wheel_pid_init(void);                                  // ��ʼ���ٶ�PID

void wheel_pid_enable(uint8 enable_output);                 // �������ʹ��

void wheel_pid_set_target_pos(float target_pos_m);          // �����ӿڼ�����

void wheel_pid_set_target_speed(float target_speed_mps);    // ����Ŀ���ٶ�

void wheel_pid_set_position_param(float kp, float ki, float kd, float i_limit); // �����ӿڼ�����

void wheel_pid_set_speed_param(float kp, float ki, float kd, float i_limit);    // �����ٶȻ�����

void wheel_pid_update(const EncoderLayerState *enc, float dt_s);                 // �ٶ�PID���ڸ���
