...
static void INS_RefreshRealtimeState(const INS_State* ins_state)
{
    ...
    if(track_follow_flag == 1)
    {
        ...
        INS.Dis_ins = sqrtf(dx * dx + dy * dy);                          // 计算到目标点的欧氏距离
    }
    else
    {
        INS.Dis_ins = 0.0f;
...
void INS_init(void)
{
    ...
    INS.sub_mode = INS_SUB_MODE_SAVE;                                    // 默认子模式为存点模式
...
void INS_Display(void)
{
    ...
    // 显示三层yaw值
    float yaw_gyro, yaw_stable, yaw_final;
    Ins_get_yaw_layers(&yaw_gyro, &yaw_stable, &yaw_final);

    ips200_show_string(0, 32, "Gyro Yaw:");
    ips200_show_float(80, 32, yaw_gyro * 57.29578f, 3, 2);

    ips200_show_string(0, 48, "Mag Yaw:");
    ips200_show_float(80, 48, yaw_stable * 57.29578f, 3, 2);

    ips200_show_string(0, 64, "Fin Yaw:");
    ips200_show_float(80, 64, yaw_final * 57.29578f, 3, 2);
