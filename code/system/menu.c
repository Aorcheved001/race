...
int key_detect(key_index_enum key_n, key_state_enum state)
{
    ...
    key_state_enum current_state = key_get_state(key_n);                  // ��ȡ��ǰ����״̬
...
void menu(void)
{
    ...
    {
        ips200_clear();
        if(++page > MAX_PAGE) page = 0;                                   // ��һҳ
    }
    if(key_detect(KEY_1, KEY_SHORT_PRESS))
    {
        ips200_clear();
        if(--page < 0) page = MAX_PAGE;                                   // ��һҳ
    }
