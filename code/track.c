...
static float pure_pursuit_calc_steer(float x, float y, float yaw, Ins_follow* target)
{
    ...
    // 2. 转换到车身坐标系
    // 假设：yaw=0时，车头朝向X轴正方向（请根据实际坐标系确认）
    ...
    if(ld < 0.1f) return 0.0f; // 建议阈值稍微大一点，比如 10cm
    ...
    while(follow_abs_index <= search_end)
    {
        ...
        if(track_flash_get_point(follow_point_idx, &pt))
        {
            ...
            if(dist >= TRACK_LOOKAHEAD_DISTANCE)
            {
                ...
                // 向上位机发送调试信息
//                printf("TrackDebug:LookaheadPoint,Index=%lu,X=%.4f,Y=%.4f,Yaw=%.4f,Dist=%.4f\r\n",
//                       (unsigned long)follow_abs_index, pt.x, pt.y, pt.yaw, dist);
    ...
    if(track_follow_prepare_point(stored_points))
    {
        ...
        if(track_flash_get_point(follow_point_idx, &pt))
        {
            ...
            float dist = sqrtf(dx * dx + dy * dy);
//            printf("TrackDebug:LookaheadPoint,Index=%lu,X=%.4f,Y=%.4f,Yaw=%.4f,Dist=%.4f\r\n",
//                   (unsigned long)stored_points, pt.x, pt.y, pt.yaw, dist);
...
static void track_follow(void)
{
    ...
    if(dist_to_lookahead < 0.05f)
    {
        ...
        {
            // 向上位机发送到达点的调试信息
//            printf("TrackDebug:ReachedPoint,Index=%lu,X=%.4f,Y=%.4f,Yaw=%.4f,Dist=%.4f\r\n",
//                   (unsigned long)follow_abs_index, Ins_date_377.x, Ins_date_377.y, Ins_date_377.yaw, dist_to_lookahead);
        ...
        {
            // 向上位机发送到达终点的调试信息
//            printf("TrackDebug:ReachedFinalPoint,Index=%lu,X=%.4f,Y=%.4f,Yaw=%.4f,Dist=%.4f\r\n",
//                   (unsigned long)stored_points, Ins_date_377.x, Ins_date_377.y, Ins_date_377.yaw, dist_to_lookahead);
...
uint8 track_flash_get_point(uint32 point_index, Ins_follow* out)
{
    ...
    base = (point_index - 1) * Track_Flash_Float_Num;          // 第point_index个点的起始地址为(point_index-1)*3
...
//    printf("TrackInfo:Start,TotalPoints=%lu\r\n", (unsigned long)stored_points);
    ...
    for(i = 1; i <= stored_points; i++)
    {
        if(track_follow_prepare_point(i))
        {
            ...
            if(track_flash_get_point(follow_point_idx, &pt))
            {
//                printf("TrackPoint:%lu,%.4f,%.4f,%.4f\r\n",
//                       (unsig ned long)i, pt.x, pt.y, pt.yaw);
            }
        }
    }
